...
namespace FinanceTrackerApp.Helpers
{
    public class Database
    {
        ...
        private void CreateDatabase()
        {
            ...
            _connection.Open();
            commandString = $"CREATE TABLE IF NOT EXISTS investing_account_info (investing_account_info_id INTEGER PRIMARY KEY, investing_account_info_name VARCHAR(100), investing_account_type INTEGER);";
            using (SQLiteCommand command = new(commandString, _connection))
            {
                command.ExecuteNonQuery();
            }
            _connection.Close();
            _connection.Open();
            commandString = $"CREATE TABLE IF NOT EXISTS investing_account (investing_account_id INTEGER PRIMARY KEY, investing_account_info_id INTEGER, investing_account_total REAL, investing_account_date VARCHAR(10), FOREIGN KEY (investing_account_info_id) REFERENCES investing_account_info (investing_account_info_id));";
            using (SQLiteCommand command = new(commandString, _connection))
            {
                command.ExecuteNonQuery();
            }
